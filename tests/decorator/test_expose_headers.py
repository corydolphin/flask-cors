...
class ExposeHeadersTestCase(FlaskCorsTestCase):
    ...
    def test_default(self):
        for resp in self.iter_responses("/test_default", origin="www.example.com"):
            self.assertTrue(resp.headers.get(ACL_EXPOSE_HEADERS) is None, "No Access-Control-Expose-Headers by default")
    ...
    def test_override(self):
        ...
        for resp in self.iter_responses("/test_override", origin="www.example.com"):
            self.assertEqual(resp.headers.get(ACL_EXPOSE_HEADERS), "X-Another-Custom-Header, X-My-Custom-Header")
