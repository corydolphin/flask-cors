...
class AllowHeadersTestCaseIntegration(FlaskCorsTestCase):
    ...
    def test_default(self):
        for resp in self.iter_responses("/test_default"):
            self.assertTrue(resp.headers.get(ACL_ALLOW_HEADERS) is None, "Default should have no allowed headers")
    ...
    def test_allow_headers_with_request_headers(self):
        ...
        """
        resp = self.preflight(
            "/test_allow_headers", origin="www.example.com", cors_request_headers=["X-Example-Header-A"]
        )
    ...
    def test_allow_headers_with_unmatched_request_headers(self):
        ...
        """
        resp = self.preflight(
            "/test_allow_headers", origin="www.example.com", cors_request_headers=["X-Not-Found-Header"]
        )
        self.assertEqual(resp.headers.get(ACL_ALLOW_HEADERS), None)
        ...
        resp = self.preflight(
            ...
            cors_request_headers=["X-Example-Header-A", "X-Not-Found-Header"],
        )
        self.assertEqual(resp.headers.get(ACL_ALLOW_HEADERS), "X-Example-Header-A")
    ...
    def test_allow_headers_regex(self):
        ...
        """
        resp = self.preflight(
            "/test_allow_headers_regex", origin="www.example.com", cors_request_headers=["X-COMPANY-FOO"]
        )
        self.assertEqual(resp.headers.get(ACL_ALLOW_HEADERS), "X-COMPANY-FOO")

        resp = self.preflight(
            "/test_allow_headers_regex", origin="www.example.com", cors_request_headers=["X-Not-Found-Header"]
        )
        self.assertEqual(resp.headers.get(ACL_ALLOW_HEADERS), None)
