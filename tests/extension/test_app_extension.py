    ...
    def setUp(self):
        ...
        CORS(
            ...
            resources={
                ...
                r"/test_set": {"origins": {"http://foo.com", "http://bar.com"}},
                r"/test_subdomain_regex": {"origins": r"http?://\w*\.?example\.com:?\d*/?.*"},
                r"/test_regex_list": {"origins": [r".*.example.com", r".*.otherexample.com"]},
                r"/test_regex_mixed_list": {"origins": ["http://example.com", r".*.otherexample.com"]},
                r"/test_send_wildcard_with_origin": {"send_wildcard": True},
                re.compile(r"/test_compiled_subdomain_\w*"): {"origins": re.compile(r"http://example\d+.com")},
                r"/test_defaults": {},
            },
        )
    ...
    def test_compiled_subdomain_regex(self):
        for sub in [1, 100, 200]:
            ...
            for resp in self.iter_responses("/test_compiled_subdomain_regex", headers={"origin": domain}):
                self.assertEqual(domain, resp.headers.get(ACL_ORIGIN))
        for resp in self.iter_responses("/test_compiled_subdomain_regex", headers={"origin": "http://examplea.com"}):
...
class AppExtensionList(FlaskCorsTestCase):
    def setUp(self):
        self.app = Flask(__name__)
        CORS(
            self.app, resources=[r"/test_exposed", r"/test_other_exposed"], origins=["http://foo.com", "http://bar.com"]
        )
    ...
    def test_exposed(self):
        for path in "/api/v1/foo", "/api/v1/bar":
            for resp in self.iter_responses(path, origin="http://bar.com"):
                self.assertEqual(resp.status_code, 200)
                self.assertEqual(resp.headers.get(ACL_ORIGIN), "http://bar.com")
                self.assertEqual(resp.headers.get(ACL_EXPOSE_HEADERS), "X-Total-Count")
...
class AppExtensionExampleApp(FlaskCorsTestCase):
    def setUp(self):
        self.app = Flask(__name__)
        CORS(self.app, resources={r"/api/*": {"origins": ["http://blah.com", "http://foo.bar"]}})
