...
class InternalsTestCase(unittest.TestCase):
    ...
    def test_flexible_str_set(self):
        self.assertEqual(flexible_str({"Foo", "Bar", "Qux"}), "Bar, Foo, Qux")
    ...
    def test_get_allow_headers_matching(self):
        ...
        self.assertEqual(get_allow_headers(options, "X-Foo, X-Bar"), "X-Bar, X-Foo")

    def test_get_allow_headers_matching_none(self):
        options = serialize_options({"allow_headers": r"X-FLASK-.*"})

        self.assertEqual(get_allow_headers(options, "X-FLASK-CORS"), "X-FLASK-CORS")
        self.assertEqual(get_allow_headers(options, "X-NOT-FLASK-CORS"), "")

    def test_parse_resources_sorted(self):
        resources = parse_resources({
            "/foo": {"origins": "http://foo.com"},
            re.compile(r"/.*"): {"origins": "http://some-domain.com"},
            re.compile(r"/api/v1/.*"): {"origins": "http://specific-domain.com"},
        })

        self.assertEqual([r[0] for r in resources], [re.compile(r"/api/v1/.*"), "/foo", re.compile(r"/.*")])
